...
int index_load(Index *index) {
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        int matched = sscanf(line, "%o %64s %llu %u %511s",
                             &entry->mode,
                             hex,
...
int index_save(const Index *index) {
    ...
    // Use heap allocation to avoid stack overflow with large Index structs.
...
// Stage a file: read its contents, write as blob, update index entry.
